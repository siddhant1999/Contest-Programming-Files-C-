...
//  KeplerCode
//
//  Created by Siddhant Jain on 2016-08-25.
...
int MAX = 1147483647;
vcc board;
vc positionsLat, positionsLon, rewards, rates, maxdatas, clengths;

//position of the data center as a function of time
...
int main(void)
{
    input;
    int num;
    //cone angle = 20 degrees above the horizon for someone looking from the ground
    cin >> num;//number of terminals (data centers)
    
    for (int i = 0; i < num; i++) {
        int lat, lon;//position coordinates
        cin >> lat >> lon;
        positionsLat.push_back(lat);
        positionsLon.push_back(lon);
        
        int rewardMB, dataRate;
        cin >> rewardMB >> dataRate;
        rewards.push_back(rewardMB);
        rates.push_back(dataRate);
        
        int maxadata; //the total amount of data a center has to be sent in the period
        int cyclelength;// in how long all this data must be trasfered
        
        cin >> maxadata >> cyclelength;
        maxdatas.push_back(maxadata);
        clengths.push_back(cyclelength);
